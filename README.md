...
AMADEUS uses the FFmpeg build pinned by `imageio-ffmpeg` to inspect and convert analysis videos. On Windows and Linux, the launcher prepares a separate checksum-pinned GPU-capable FFmpeg build when it detects a GPU; existing installs prepare it when Cropping & Trimming is first opened. NVIDIA NVENC, Intel Quick Sync, AMD AMF, and Linux VAAPI are tested before the hardware encoding option is enabled. The final H.264 encoding uses the GPU; crop, rotation, and image adjustments still run on the CPU. If no hardware encoder works, exports use CPU encoding. See [third-party notices](THIRD_PARTY_NOTICES.md).
