...
AMADEUS uses the FFmpeg build pinned by `imageio-ffmpeg` to inspect and convert analysis videos. Cropping & Trimming checks for a working hardware encoder; on Windows and Linux GPU systems, it downloads a checksum-pinned FFmpeg build the first time the bundled build cannot use the detected GPU. NVIDIA NVENC, Intel Quick Sync, and AMD AMF are tested before the hardware encoding option is enabled. The final H.264 encoding uses the GPU; crop, rotation, and image adjustments still run on the CPU. If no hardware encoder works, exports use CPU encoding. See [third-party notices](THIRD_PARTY_NOTICES.md).
