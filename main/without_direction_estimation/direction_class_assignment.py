...
def get_min_obb_aspect_ratio(*args, **kwargs):
    return 1.0
...
def classify_jump_outliers_stage1(*args):
    return {'jump_bounds': (0.0, float('inf')), 'jump_threshold_px': float('inf')}
...
def classify_blobs_pre_direction(blob_records, cfg):
    kept = 0
    for items in blob_records.values():
        for blob in items:
            blob.erase_pre_direction = blob.erase_stage1
            blob.erase_reasons_pre_direction = list(blob.erase_reasons_stage1)
            kept += not blob.erase_pre_direction
    return {'pre_direction_unmatched': 0, 'pre_direction_short_run': 0,
            'pre_direction_kept': kept}


def estimate_directions(blob_records, cfg, individual_distance_px):
    """Use an unsigned OBB axis only to transform donor geometry.

    The shared donor container calls these fields direction_vec/axis_length.
    Here they describe geometry, never a movement-derived head or tail.
    """
    import numpy as np
    kept = 0
    for items in blob_records.values():
        for blob in items:
            blob.erase_final = blob.erase_pre_direction
            blob.erase_reasons_final = list(blob.erase_reasons_pre_direction)
            if blob.erase_final:
                continue
            fit_mode = normalize_obb_fit_mode(cfg.get("OBB_FIT_MODE", "min_area"))
            pts = contour_to_obb_points(blob.contour, fit_mode)
            edges = np.roll(pts, -1, axis=0) - pts
            lengths = np.linalg.norm(edges, axis=1)
            if not np.isfinite(lengths).all() or min(lengths) <= 0:
                raise ValueError(f'Degenerate OBB: frame={blob.frame}, blob={blob.blob_index}')
            if fit_mode == "pca":
                axis = obb_axis_from_contour(blob.contour, fit_mode)
                if axis is None:
                    raise ValueError(f'Degenerate PCA axis: frame={blob.frame}, blob={blob.blob_index}')
                axis_length = _obb_geometry_from_contour(blob.contour, fit_mode)[1]
            else:
                index = int(np.argmax(lengths))
                axis = edges[index] / lengths[index]
                axis_length = float(lengths[index])
            blob.class_id = 0
            blob.direction_vec = tuple(map(float, axis))
            blob.axis_length = float(axis_length)
            blob.direction_failure_reasons.clear()
            kept += 1
    return {'direction_input_blobs': kept, 'valid_segments': 0,
...
def draw_heading_triangle_for_obb(*args, **kwargs):
    return None
