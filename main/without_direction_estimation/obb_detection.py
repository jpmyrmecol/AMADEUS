...
from without_direction_estimation.training_paths import OUTPUT_ROOT_DIR, training_weight_path
...
def class_name_from_direction(*args, **kwargs):
    return ''
...
def normalize_candidate_df_direction(df, reference_is_yolo_class):
    """Retain detections regardless of OBB angle; no heading is inferred."""
    import numpy as np
    import pandas as pd
    if df is None:
        return make_empty_candidate_df()
    work = df.copy()
    work['direction'] = np.nan
    if work.empty:
        return work
    coordinates = [f'{axis}{i}' for axis in ('x', 'y') for i in range(4)]
    valid = np.isfinite(work[coordinates].to_numpy(dtype=float)).all(axis=1)
    work = work[valid].sort_values(['frame', 'score', 'source_weight', 'det_index'],
...
def collect_candidate_df(
    ...
    if names != ['animal']:
        raise ValueError('Without direction estimation requires a single-class animal model; retrain in this mode.')
    cap = cv2.VideoCapture(video_path)
