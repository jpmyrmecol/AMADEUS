...
def predict_obb_from_filters(prev_obb, position_kf, bbox_shape_kf, direction_kf=None):
    """Translate/resize the previous rectangle without a heading filter."""
    rect = obb_points_to_rect(prev_obb)
    center = rect[:2] if position_kf is None else position_kf.x[:2].reshape(2)
    size = rect[2:4] if bbox_shape_kf is None else bbox_shape_kf.x[:2].reshape(2)
    return cv2.boxPoints((tuple(map(float, center)), tuple(map(float, np.maximum(size, 1e-6))), float(rect[4]))).astype(np.float32)
...
def final_result_df_from_buffers(buffers: dict, num_objects: int) -> pd.DataFrame:
    ...
    for tid in ids:
        block = np.full((len(frames), 4), np.nan, dtype=float)
        ...
        for col_idx, name in enumerate(('cx', 'cy', 'w', 'h')):
...
def _track_detection_pass(
    ...
    *,
    ...
    direction_kfs = [None] * num_objects  # no direction filter is ever instantiated
    ...
    for processing_idx, abs_fid in enumerate(
        ...
        yolo_directions = np.full(len(yolo_obbs), np.nan)
        ...
        for stage_num, req_iou, req_dir, add_dir, dir_mode in (
            (ATYPE_S1, True, False, False, 'none'),
            (ATYPE_S3, False, False, False, 'none'),
        ):
