...
def get_dir(buffers: dict, tid: int, frame: int) -> float | None:
    raw = buffers.get('class_buf', {}).get(tid, {}).get(frame)
    if raw is None:
        return None
    v = float(raw[0] if isinstance(raw, (tuple, list)) else raw)
    return v if np.isfinite(v) else None


def _direction_on_long_axis(
    pts: np.ndarray,
    direction: float | None,
    *,
    max_axis_error_deg: float = 45.0,
) -> float | None:
    """Project direction to the OBB long axis only when it is close enough."""
    ...
    projected = float(unit_vec_to_angle_deg(closest_long_axis_direction(pts, ref)))
    if angular_distance_deg(ref, projected) <= float(max_axis_error_deg):
        return projected
    return None
...
def _apply_embedding_rescue(
    ...
    """
    from identity_correction import EpisodeDecision, attempt_embedding_rescue
...
def _apply_one_sided_rescue(
    ...
    (track_end/seam/apply/relabel) to apply unchanged.
    """
    from identity_correction import EpisodeDecision, attempt_one_sided_rescue
...
def _eval_kalman_gap_fill_id(
    ...
    dir_f0 = get_dir(corrected, tid, f0)
    center_f0 = obb_center(obb_f0).astype(float)
    rect_f0 = obb_points_to_rect(obb_f0)
    dir_vec_f0 = closest_long_axis_direction(obb_f0, dir_f0)
    ...
    dir_kf = create_direction_kf(dir_vec_f0)

    last_valid_t = f0
    last_valid_obb_v = obb_f0
    last_valid_dir_v = dir_f0
    in_gap = False
    ...
    for t in range(f0 + 1, f1 + 1):
        ...
        if is_valid:
            ...
            if in_gap:
                ...
                dir_a = last_valid_dir_v
                obb_b = obb_t
                dir_b = dir_t

                # KF-predicted OBB/direction at t_b (after gap_len predict steps)
                pred_obb_tb = predict_obb_from_filters(last_valid_obb_v, pos_kf, shape_kf, dir_kf)
                pred_dir_tb = unit_vec_to_angle_deg(direction_vec_from_kf(dir_kf))
                ...
                if g1 and g2:
                    ...
                    if branch is not None:
                        # G4: direction deviation for the chosen branch
                        if branch == 'prediction':
                            if pred_dir_tb is not None and dir_b is not None:
                                dir_dev = angular_distance_deg(pred_dir_tb, dir_b)
                                g4 = dir_dev <= strong_dir_deg
                            else:
                                dir_dev = 0.0
                                g4 = True  # skip when dir unavailable (conservative)
                        else:
                            if dir_a is not None and dir_b is not None:
                                dir_dev = angular_distance_deg(dir_a, dir_b)
                                g4 = dir_dev <= strong_dir_deg
                            else:
                                dir_dev = 0.0
                                g4 = True
                        ...
                        if g4:
                            # Build the entire chord first.  A finite direction
                            # that is more than 45 degrees from an interpolated
                            # OBB's long axis invalidates the whole gap.
                            a_pts = ensure_clockwise(obb_a)
                            b_pts = canonicalize_obb_points(obb_b, reference=a_pts)
                            gap_fills: list[tuple] = []
                            interp_reject: dict | None = None
                            for t_fill in range(t_a + 1, t_b):
                                ...
                                if dir_a is not None and dir_b is not None:
                                    dir_fill: float | None = circular_interpolate_deg(dir_a, dir_b, alpha)
                                elif dir_a is not None:
                                    dir_fill = dir_a
                                elif dir_b is not None:
                                    dir_fill = dir_b
                                else:
                                    dir_fill = None
                                projected_dir = _direction_on_long_axis(
                                    pts_fill, dir_fill,
                                    max_axis_error_deg=max_axis_error_deg,
                                )
                                if dir_fill is not None and projected_dir is None:
                                    nearest_axis = float(unit_vec_to_angle_deg(
                                        closest_long_axis_direction(pts_fill, dir_fill)
                                    ))
                                    interp_reject = {
                                        'frame': t_fill,
                                        'interp_dir': float(dir_fill),
                                        'obb_axis_dir': nearest_axis,
                                        'axis_dir_dev': angular_distance_deg(dir_fill, nearest_axis),
                                    }
                                    break
                                gap_fills.append((t_fill, pts_fill, projected_dir))

                            if interp_reject is None:
                                fills.extend(gap_fills)
                                local_log.append({
                                    'type': 'kf_fill',
                                    'action': 'fill',
                                    'ids': [tid],
                                ...
                                local_log.append({
                                    'type': 'kf_fill',
                                    'action': 'reject_interp_direction',
                                    'ids': [tid],
                                    'frame_start': t_a + 1,
                                    'frame_end': t_b - 1,
                                    'gap_len': gap_len,
...
def _eval_position_spikes_id(
    ...
    for k in range(1, len(valid) - 1):
        ...
        dir_prev = get_dir(corrected, tid, t_prev)
        dir_next = get_dir(corrected, tid, t_next)
        if dir_prev is None or dir_next is None:
            continue
        current_dir = get_dir(corrected, tid, t)
        if _is_trusted_anchor(corrected, tid, t) and current_dir is not None:
            direction = current_dir
        else:
            direction = circular_interpolate_deg(dir_prev, dir_next, alpha)

        new_dir = _direction_on_long_axis(
            new_obb,
            direction,
            max_axis_error_deg=max_axis_error_deg,
        )
        if new_dir is None:
            # Preserve the frame if the interpolated OBB cannot be assigned
            # a direction consistent with its long axis.
            continue
...
def _fix_direction_segment_flips(
    corrected: dict,
    num_objects: int,
    *,
    num_workers: int,
    ...
    """Resolve single- and multi-frame 180-degree direction flips by anchor-run cost.

    The read-only evaluation is parallelized by track ID.  Only non-anchor runs
    with at least one immediate reliable neighboring anchor are eligible for
    correction; otherwise the run is preserved to avoid over-correction.
    """
    results = _run_id_eval(
        corrected,
        num_objects,
        num_workers,
        desc=f'Fix direction segment flips [{phase}]',
        serial_fn=_eval_direction_segment_flips_id,
        worker_args=(
            phase,
            max_axis_error_deg,
            flip_frame_threshold,
        ),
        buffer_names=('obb_buf', 'class_buf'),
        concurrent_jobs=concurrent_jobs,
    )

    log: list[dict] = []
    n_flips = 0
    for tid, (changes, logs) in enumerate(results):
        for t, new_dir in changes:
            corrected.setdefault('class_buf', {}).setdefault(tid, {})[t] = (new_dir,)
            corrected.setdefault('direction_corrected_buf', {}).setdefault(tid, {})[t] = (1.0,)
            _record_assign_type(
                corrected, tid, t, ATYPE_DIR_FLIP, 'correction_direction_segment_flip',
            )
        n_flips += sum(1 for e in logs if e.get('action') == 'segment_flip')
        log.extend(logs)

    abstains = sum(1 for e in log if e.get('action') == 'abstain')
    print(
        f'Fix direction segment flips [{phase}]: '
        f'num corrections={n_flips}, abstain={abstains}',
        flush=True,
    )
    return sorted(
        log,
        key=lambda entry: (
            int(entry['ids'][0]),
            int(entry.get('frame_start', 0)),
            0 if entry.get('action') == 'segment_flip' else 1,
        ),
    )
