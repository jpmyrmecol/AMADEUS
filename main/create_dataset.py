...
def rotate_heading_class(class_id: int, rotation_deg: int) -> int:
    """Rotate a heading class by re-quantizing its rotated direction vector.

    Deliberately does not add rotation_deg // 45 to class_id directly: the
    heading class carries head/tail polarity that an OBB's long axis alone
    cannot recover, so the rotation must go through the direction vector.
    For right-angle rotations this is equivalent to (class_id + rotation_deg
    // 45) % 8, which is asserted by tests.
    """
    dx, dy = class_id_to_unit_vec(class_id)
    dx2, dy2 = rotate_direction_vec_right_angle(dx, dy, rotation_deg)
    return angle_to_class_id(dx2, dy2)
...
def write_rotation_dataset_previews(
    ...
    from apply_class_label_filtering import draw_preview_from_label_lines
