...
from obb_detection import iou_obb
...
def build_fragments(corrected: dict, num_objects: int, num_workers: int = 1) -> list[Fragment]:
    """Return isolated heading-stable runs of length >= 4 for each object."""
    obb_buf = corrected['obb_buf']
    dir_buf = corrected['class_buf']
    num_w = _worker_count(num_workers, num_objects)

    # [A] Precompute isolation per (obj_id, frame) once with AABB pruning.
    isolation_map = _isolation_by_frame(obb_buf, dir_buf, num_objects)

    if num_w <= 1:
        per_object = [
            _build_fragments_for_object(obb_buf, dir_buf, obj_id, num_objects, isolation_map)
            for obj_id in tqdm_it(range(num_objects), desc='Embedding build fragments', unit='id')
        ]
    else:
        with ThreadPoolExecutor(max_workers=num_w) as ex:
            iterator = ex.map(
                lambda obj_id: _build_fragments_for_object(obb_buf, dir_buf, obj_id, num_objects, isolation_map),
                range(num_objects),
            )
            per_object = list(tqdm_it(iterator, total=num_objects, desc='Embedding build fragments', unit='id'))

    fragments: list[Fragment] = []
    frag_id = 0
    for obj_fragments in per_object:
        for frag in obj_fragments:
