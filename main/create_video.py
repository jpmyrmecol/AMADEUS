...
def ellipse_parameters_from_obb(pts: np.ndarray) -> tuple[np.ndarray, np.ndarray, float, float]:
    """Return (center, major_axis, major_length, minor_length) for an OBB ellipse.

    The OBB side lengths are used as ellipse diameters, so the ellipse is fitted
    inside the OBB and retains its center, rotation, and aspect ratio.
    """
...
def draw_ellipse_arrowhead(
    ...
    """Fill the front ellipse arc and its two tangent segments as an arrowhead.

    The tip is one-third of the full ellipse major-axis length beyond the
    ellipse's front point.  For an ellipse x²/a² + y²/b² = 1 and an apex at
    (d, 0), the tangent points are (a²/d, ±b*sqrt(1-a²/d²)).
    """
    center, major_axis, major_length, minor_length = ellipse_parameters_from_obb(pts)
    if (
        major_length <= 0.0
        or minor_length <= 0.0
        or not np.isfinite(major_axis).all()
        or not np.isfinite(float(direction))
    ):
        return

    direction_vec = angle_to_unit_vec(float(direction))
    if float(np.dot(major_axis, direction_vec)) < 0.0:
        major_axis = -major_axis
    minor_axis = np.array([-major_axis[1], major_axis[0]], dtype=float)
    semi_major = major_length / 2.0
    semi_minor = minor_length / 2.0
    arrow_length = major_length / 3.0
    tip_distance = semi_major + arrow_length
    if semi_major <= 0.0 or semi_minor <= 0.0 or tip_distance <= semi_major:
        return

    tangent_x = (semi_major * semi_major) / tip_distance
    tangent_ratio = semi_major / tip_distance
    tangent_y = semi_minor * math.sqrt(max(0.0, 1.0 - tangent_ratio * tangent_ratio))
    tangent_angle = math.acos(float(np.clip(tangent_x / semi_major, -1.0, 1.0)))
    arc_points = []
    for theta in np.linspace(tangent_angle, -tangent_angle, 25):
        local = np.array([semi_major * math.cos(theta), semi_minor * math.sin(theta)], dtype=float)
        arc_points.append(center + major_axis * local[0] + minor_axis * local[1])
    tip = center + major_axis * tip_distance
    polygon = np.vstack([tip, np.asarray(arc_points, dtype=float)])
    polygon_i32 = np.round(polygon).astype(np.int32).reshape(-1, 1, 2)

    overlay = img.copy()
    cv2.fillPoly(overlay, [polygon_i32], color, lineType=cv2.LINE_AA)
    alpha = float(np.clip(alpha, 0.0, 1.0))
    cv2.addWeighted(overlay, alpha, img, 1.0 - alpha, 0.0, dst=img)
    if int(outline_thickness) > 0:
        cv2.polylines(img, [polygon_i32], True, color, int(outline_thickness), cv2.LINE_AA)
...
def _render_video_frame(
    ...
    for tid in ctx.ids:
        ...
        if ctx.draw_arrow and cls_idx is not None:
            if ctx.drawing_mode == "ellipse":
                draw_ellipse_arrowhead(
                    frame, pts, cls_idx, color,
                    ctx.triangle_alpha, ctx.triangle_outline_thickness,
                )
            else:
                draw_direction_triangle_for_obb(
                    frame, pts, cls_idx, color,
